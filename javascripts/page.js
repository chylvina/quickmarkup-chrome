...
var page = {
  ...
  hookBodyScrollValue:function (needHook) {
    document.documentElement.setAttribute(
        "__qm_need_hook_scroll_value__", needHook);
    var event = document.createEvent('Event');
    event.initEvent('__qm_check_hook_status_event__', true, true);
  ...
  addMessageListener:function () {
    ...
    chrome.extension.onConnect.addListener(function (port) {
      port.onMessage.addListener(function (msg) {
        switch (msg.msg) {
          case 'scroll_next':
            page.visibleWidth = msg.visibleWidth;
            page.visibleHeight = msg.visibleHeight;

            port.postMessage(page.scrollNext());
            break;
        }
      });
    });
  ...
  createFloatLayer:function () {
    page.createDiv(document.body, 'quickmarkup_area_protector');
  },
  ...
  createSelectionArea:function () {
    var areaProtector = $('quickmarkup_area_protector');
    ...
    page.createDiv(areaProtector, 'quickmarkup_shadow_top');
    page.createDiv(areaProtector, 'quickmarkup_shadow_bottom');
    page.createDiv(areaProtector, 'quickmarkup_shadow_left');
    page.createDiv(areaProtector, 'quickmarkup_shadow_right');

    var areaElement = page.createDiv(areaProtector, 'quickmarkup_area');
    page.createDiv(areaElement, 'quickmarkup_container');
    page.createDiv(areaElement, 'quickmarkup_size');

    //-----------------------------
    var btn = page.createDiv(areaElement, 'quickmarkup_button');
    btn.innerHTML = '<a href="javascript:void(0)" id="quickmarkup_cancel"></a><a href="javascript:void(0)" id="quickmarkup_crop">'
        + chrome.i18n.getMessage('selectedCaptureOK') + '</a>';

    page.createDiv(areaElement, 'quickmarkup_north_west');
    page.createDiv(areaElement, 'quickmarkup_north_east');
    page.createDiv(areaElement, 'quickmarkup_south_east');
    page.createDiv(areaElement, 'quickmarkup_south_west');

    //config events
    document.getElementById("quickmarkup_cancel").addEventListener('mousedown', page.removeSelectionArea, false);
    document.getElementById("quickmarkup_crop").addEventListener('mousedown', page.doCaptureSelection, false);
    areaProtector.addEventListener('mousedown', page.onMouseDown, false);
    $('quickmarkup_container').addEventListener('dblclick', page.doCaptureSelection, false);
    window.addEventListener("keydown", page.onKeyDown, false);

    page.pageHeight = $('quickmarkup_area_protector').clientHeight;
    page.pageWidth = $('quickmarkup_area_protector').clientWidth;

    var areaElement = $('quickmarkup_area');
  ...
  onKeyDown:function (event) {
    switch (event.keyCode) {
      case 27:    //ESC
        ...
        break;
      case 13:
        page.doCaptureSelection();
        break;
    }
  },

  doCaptureSelection:function () {
    page.removeSelectionArea();
    chrome.extension.sendRequest({ msg:'capture_selected' });
  },
  // ---------------------------------------
      ...
      if (element) {
        ...
        if (elementName && document) {
          ...
          if (areaElement) {
            ...
            setTimeout(page.onEnterFrame, 20);
            // ------------------
  ...
  onMouseMove:function () {
    page.mouseX = event.pageX;
    page.mouseY = event.pageY;
    page.screenX = event.clientX;
    page.screenY = event.clientY;
  },

  onScroll:function () {
    page.mouseX = page.screenX + window.scrollX;
    page.mouseY = page.screenY + window.scrollY;
  },
  ...
  onEnterFrame:function () {
    if (page.isMouseDown) {
      ...
      if (areaElement) {
        // -------------
        var xPosition = page.mouseX;
        var yPosition = page.mouseY;
        // -------------
        ...
        // -------------
        var buttons = document.getElementById('quickmarkup_button');
        if (yPosition + 30 < document.body.clientHeight) {
        ...
        var dragSizeContainer = document.getElementById('quickmarkup_size');
        if (page.mouseY < 18) {
  ...
  clearEvents:function () {
    ...
    window.removeEventListener('scroll', page.onScroll, false);
  },
  // --------------------------
  ...
  updateShadow:function (areaElement) {
    $('quickmarkup_shadow_top').style.height =
        parseInt(areaElement.style.top) + 'px';
    $('quickmarkup_shadow_top').style.width = (parseInt(areaElement.style.left) +
        parseInt(areaElement.style.width) + 1) + 'px';
    $('quickmarkup_shadow_left').style.height =
        (page.pageHeight - parseInt(areaElement.style.top)) + 'px';
    $('quickmarkup_shadow_left').style.width =
    ...
    $('quickmarkup_shadow_right').style.height = height + 'px';
    $('quickmarkup_shadow_right').style.width = width + 'px';
    ...
    width = (width < 0) ? 0 : width;
    $('quickmarkup_shadow_bottom').style.height = height + 'px';
    $('quickmarkup_shadow_bottom').style.width = width + 'px';
  },

  // ---------------------------------
  removeSelectionArea:function () {
    //clear events
    $('quickmarkup_area_protector').removeEventListener('mousedown', page.onMouseDown);
    $('quickmarkup_container').removeEventListener('dblclick', page.doCaptureSelection);
    document.getElementById("quickmarkup_cancel").removeEventListener('mousedown', page.removeSelectionArea);
    document.getElementById("quickmarkup_crop").removeEventListener('mousedown', page.doCaptureSelection);
    window.removeEventListener("keydown", page.onKeyDown);

    page.removeElement('quickmarkup_area_protector');
    page.removeElement('quickmarkup_area');
  ...
  updateSize:function () {
    ...
    $('quickmarkup_size').innerText = page.calculateSizeAfterZooming(width) +
  ...
  init:function () {
    // -------------------------------
    if (document.body.hasAttribute('quick-markup_injected')) {
      return;
    }
    // -------------------------------
    if (isPageCapturable()) {
    ...
    this.injectCssResource('stylesheets/style.css');
    this.addMessageListener();
    this.injectJavaScriptResource("javascripts/page_context.js");
